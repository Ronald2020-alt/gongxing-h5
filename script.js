...
(function () {
  ...
  const CITIES = '宁波 · 哈尔滨 · 郑州 · 合肥 · 南昌 · 赣州 · 兰州 · 西宁';
  ...
  TEMPLATES.forEach((t) => {
    ...
    item.innerHTML = '<div class="tpl-thumb">' + thumbHTML(t.id) + '</div>'
  ...
  function thumbHTML(id) {
    const bar = (w, h, c, mt) =>
      '<div style="width:' + w + '%;height:' + h + 'px;background:' + c + ';margin-top:' + mt + 'px;border-radius:2px;"></div>';
    const gold = '#d9b45f', white = 'rgba(255,255,255,.9)';
    let inner = '';
    if (id === 2) {
      inner = bar(55, 4, white, 8) + bar(40, 3, 'rgba(255,255,255,.6)', 5) + bar(80, 7, white, 16) + bar(70, 7, white, 6) + bar(60, 8, gold, 22);
    } else if (id === 3) {
      inner = bar(40, 4, white, 14) + bar(90, 11, white, 12) + bar(55, 7, white, 20);
    } else if (id === 4) {
      inner = '<div style="height:calc(100% - 12px);margin:6px;border:1.5px solid rgba(217,180,95,.7);padding:8px;box-sizing:border-box;">'
        + bar(80, 5, white, 4) + bar(85, 9, white, 14) + bar(70, 8, gold, 20) + '</div>';
    } else if (id === 5) {
      inner = bar(80, 4, white, 10) + bar(85, 6, white, 18) + bar(70, 12, gold, 26);
    } else {
      inner = bar(55, 4, white, 8) + bar(80, 8, white, 16) + bar(65, 7, white, 6) + bar(70, 9, gold, 18);
    }
  ...
  nodeSelect.addEventListener('change', () => {
    ...
    if (n.id === 'custom') {
      nodeCustom.hidden = false;
    } else {
      nodeCustom.hidden = true;
      $('#tributeLine1').value = n.l1;
      $('#tributeLine2').value = n.l2;
      $('#tributeEn').value = n.en;
    }
    renderTribute();
  });

  // ---------- 文案字段实时渲染 ----------
  const fields = ['quote', 'shop', 'dept', 'name', 'scene', 'tributeLine1', 'tributeLine2', 'tributeEn'];
  fields.forEach((f) => {
    const el = document.getElementById(f);
    el.addEventListener('input', () => {
      if (f.startsWith('tribute')) renderTribute();
      else renderText();
    });
  });
  ...
  photoInput.addEventListener('change', (e) => {
    ...
    reader.onload = (ev) => {
      ...
      img.onload = () => {
        ...
        photoState.nw = img.naturalWidth;
        photoState.nh = img.naturalHeight;
        applyPhotoToDOM();
        fitCover();
  ...
  function applyPhotoToDOM() {
    ...
    pPhoto.style.objectPosition = 'initial';
    updatePhotoTransform();
  }

  function fitCover() {
    const s = Math.max(PW / photoState.nw, PH / photoState.nh);
    photoState.baseScale = s;
    photoState.scale = s;
  ...
  posterWrap.addEventListener('pointerup', (e) => {
    drag = null; posterWrap.style.cursor = 'grab';
  });
  posterWrap.addEventListener('wheel', (e) => {
    if (!adjusting) return;
    e.preventDefault();
    zoomPhoto(e.deltaY < 0 ? 1.06 : 0.94, e.clientX, e.clientY);
  }, { passive: false });
  ...
  posterWrap.addEventListener('touchmove', (e) => {
    ...
    e.preventDefault();
    const ns = pinchStart.scale * (dist(e.touches) / pinchStart.d);
    setScale(ns, pinchStart.cx, pinchStart.cy);
  }, { passive: false });
  ...
  function zoomPhoto(factor, cx, cy) {
    setScale(photoState.scale * factor, cx, cy);
  }
  function setScale(ns, cx, cy) {
    ...
    const sx = PW / rect.width, sy = PH / rect.height; // 逻辑->屏幕比例
    // 指针在画布逻辑坐标
  ...
  async function download() {
    ...
    try {
      ...
      // 2. 临时替换照片（铺满、无 transform），避免导出 transform 兼容问题
      const orig = { src: pPhoto.src, w: pPhoto.style.width, h: pPhoto.style.height, tf: pPhoto.style.transform };
      pPhoto.style.width = PW + 'px';
      pPhoto.style.height = PH + 'px';
      pPhoto.style.objectFit = 'cover';
      pPhoto.style.transform = 'none';
      pPhoto.src = bgUrl;
      await new Promise((r) => { pPhoto.onload = r; if (pPhoto.complete) r(); });

      // 3. 主KV渐变文字在导出时降级为纯金色（避免 background-clip:text 导出异常）
      const kv = document.querySelector('.p-kv-text');
      ...
      // 5. 恢复
      kv.style.background = '';
      kv.style.webkitTextFillColor = '';
      kv.style.color = '';
      pPhoto.src = orig.src;
      pPhoto.style.width = orig.w;
      pPhoto.style.height = orig.h;
      pPhoto.style.objectFit = 'none';
      pPhoto.style.transform = orig.tf;

      // 6. 下载
      const shop = $('#shop').value.trim() || '门店';
      const name = $('#name').value.trim() || '人物';
      ...
      btnDownload.textContent = '⬇ 生成并下载海报';
    }
  }
  ...
  // 等待字体后重绘
  document.fonts.ready.then(() => { /* 字体就绪 */ });
  init();
})();
